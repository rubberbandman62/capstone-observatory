...
object Extraction {
  ...
  def locateTemperatures(year: Year, stationsFile: String, temperaturesFile: String): Iterable[(LocalDate, Location, Temperature)] = {
    val locatedtemperatruesRDD = myLocateTemperatures(year, stationsFile, temperaturesFile)
    val persisted = locatedtemperatruesRDD.persist(StorageLevel.MEMORY_AND_DISK_SER)
    persisted.collect()
  }
  ...
  def myLocateTemperatures(year: Year, stationsFile: String, temperaturesFile: String): RDD[(LocalDate, Location, Temperature)] = {
    ...
    val temperaturesRDD = rawTemperatures.map(line => line.split(","))
      .map(arr => ((arr(0).trim, arr(1).trim), (arr(2).toInt, arr(3).toInt, (arr(4).toDouble - 32) / 1.8d)))
  ...
  def myLocationYearlyAverageRecords(records: RDD[(LocalDate, Location, Temperature)]): RDD[(Location, Temperature)] = {
    records.map({ case (localdate, location, temp) => ((localdate.getYear, location), (temp, 1)) })
      ...
      .map({ case (key, value) => (key._2, value._1 / value._2) }).persist(StorageLevel.MEMORY_AND_DISK)
  }
